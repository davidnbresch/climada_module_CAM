function tc_track=climada_tc_read_cam_database(cam_data_dir,starty,endy)
...
if isempty(cam_data_dir) % ask for CAM data folder
    ...
    cam_data_dir=[climada_global.root_dir '_additional' filesep 'CAM' filesep 'data' filesep 'track_data'];
...
if findstr(fN,'_V01') % check for version
    fprintf('NOTE: if you encounter troubles, consider using climada_tc_read_cam_database_V01 instead\n');
end
...
raw_data=NaN(1,12); % Create first line to access
...
for i=starty:endy
    ...
    while not(feof(fid))
        ...
        if strcmp(str,'start')==1
            ...
            % NEW : 12 fields
            raw_data(end+1:end+nsteps,:) = fscanf(fid,'%f',[12 nsteps])';
        else
        end
    end
end
...
%------------------------------------------------------
raw_data(raw_data(:,8) > 1100,4)=NaN; % replace silly pressure data
raw_data(raw_data(:,8) <  800,4)=NaN; % replace silly pressure
...
for i=1:n_tracks
    ...
    tc_track(i).CentralPressure=raw_data(ind,8);
    tc_track(i).yyyy=raw_data(ind,9);
    tc_track(i).mm=raw_data(ind,10);
    tc_track(i).dd=raw_data(ind,11);
    tc_track(i).hh=raw_data(ind,12);
    tc_track(i).ID_no=basin_number*1e6+raw_data(ind(1),1)*100+storm_yr(i);
    tc_track(i).orig_event_flag=1;
    tc_track(i).datenum=datenum(raw_data(ind,9),raw_data(ind,10),raw_data(ind,11),raw_data(ind,12),0,0);
